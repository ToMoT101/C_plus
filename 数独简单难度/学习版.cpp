...
int main()  
{
	...
	//  ——>可以在num_alternative_repeat()函数里查看大库，现为“开启”状态


	cout << endl;
	//************************************************  用于算法判断  *********************************************************

	//bool flag_repeat, flag_row, flag_line;  // 三个条件
	//bool flag_end;  //使循环结束
	//flag_end = flag_repeat = flag_row = flag_line = false;
	/*library_row_2(library_repeat, 2, num_0, position);
	library_line_2(library_repeat, 2, num_0, position);
	library_row_2(library_repeat, 21, num_0, position);
	library_line_2(library_repeat, 21, num_0, position);
	library_row_2(library_repeat, 22, num_0, position);
	library_line_2(library_repeat, 22, num_0, position);*/
	/*for (int i = 0; i < num_0; i++)
	{
		library_line_2(library_repeat, i, num_0, position);
	}*/
	// library_line_2(library_repeat, 23, num_0, position);
	...
	while(num_0_reduce != 38)
	{
		for (int i = 0; i < num_0 ; i++)  //“只算”库只有一个数，且处于第一个位置的数，即能直接填入的数
		{
			...
			if (arr_long == 1)       // 判断是不是只有一个数字
			{
				if (library_repeat[i][0] != 0)		  //判断是不是在第一个位置
				{
					...
					library_repeat[i][0] = 0;
					//free(library_repeat[i]);//填完后释放该库的内存
		...
		{
			int alternative = arr_num(library_repeat[j]);
			for (int k = 0; k < alternative; k++)
			{
				cout << library_repeat[j][k] << "    ";
			}
			cout << endl;
		}
		cout << endl;
		...
		for (int j = 0; j < num_0; j++)
		{
			...
			for (int k = 0; k < alternative; k++)
			{
				cout <<j<<"->"<< position[j] << "->" << library_repeat[j][k] << "    ";
			}
			cout << endl;
		}
		cout << endl;
